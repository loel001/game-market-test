...
describe('PurchaseForm', () => {
    ...
    describe('Game with age restriction', () => {
        ...
        it('returns selectable users', async () => {
            ...
            expect(await findByTestId(`user${USER.id}incorrectAge`)).not.toBeChecked();

            await act(async () => {
                fireEvent.click(await findByTestId(`user${NO_AGE_USER_ID}`));
            });

            expect(await findByTestId(`user${NO_AGE_USER_ID}noAge`)).not.toBeChecked();

            await act(async () => {
                fireEvent.click(await findByTestId(`user${MIN_AGE_USER_ID}`));
            });

            expect(await findByTestId(`user${MIN_AGE_USER_ID}incorrectAge`)).not.toBeChecked();
